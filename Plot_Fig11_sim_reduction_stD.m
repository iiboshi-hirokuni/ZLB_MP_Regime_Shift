...
for j = 2:nsim
  ...
  d=0;  
      ...
      if shock_g == 1
        g_sim(j) = state+sigma_g*shock(2,j);
      end
  ...
  i_sim(j) = interp2(u, g, PF_i, u_sim(j), g_sim(j), 'linear')*100;
   ...
   if isnan(pi_sim(j))==1|isnan(i_sim(j))==1|isnan(y_sim(j))==1 | y_sim(j)<sub_y
       u_sim(j) = 0;
       g_sim(j) = state+sigma_g*randn(1,1);
       pi_sim(j) = (interp2(u, g, PF_pi, u_sim(j), g_sim(j), 'linear')-pi0)*100;
       y_sim(j) = interp2(u, g, PF_y, u_sim(j), g_sim(j), 'linear')*100;
       i_sim(j) = interp2(u, g, PF_i, u_sim(j), g_sim(j), 'linear')*100;
%         display('Nan')
...
for j = 2:nsim
  ...
  d=0;  
     ...
     if shock_u == 1
       u1_sim(j) = 0; % state +sigma_u*shock(1,j);
     end
     if shock_g == 1
       g1_sim(j) =state + 0.8*sigma_g*shock(2,j);
     end
  ...
  i1_sim(j) = interp2(u1, g1, PF_i, u1_sim(j), g1_sim(j), 'linear')*100;
   ...
   if isnan(pi1_sim(j))==1|isnan(i1_sim(j))==1|isnan(y1_sim(j))==1 | y1_sim(j)<sub_y
       u1_sim(j) = 0;
       g1_sim(j) = state + 0.8*sigma_g*randn(1,1);
       pi1_sim(j) = (interp2(u1, g1, PF_pi, u1_sim(j), g1_sim(j), 'linear')-pi1)*100;
       y1_sim(j) = interp2(u1, g1, PF_y, u1_sim(j), g1_sim(j), 'linear')*100;
       i1_sim(j) = interp2(u1, g1, PF_i, u1_sim(j), g1_sim(j), 'linear')*100;
%         display('Nan')
...
[density,x2]=ksdensity(y1_sim);
   hold on
       ...
       plot(x2,density,'LineStyle',':','Color','r',...
        'LineWidth',2.5); title('Output','FontSize',14 ) ;
    xlim([-10 2.5]);    
            legend('Original','20% Reduction' )
      hold off 
subplot(1,3,2); [density,x1]=ksdensity(pi_sim);
        [density,x2]=ksdensity(pi1_sim);
   hold on
        plot(x1,density,'LineStyle','-','Color','b',...
        'LineWidth',2.5);  
       plot(x2,density,'LineStyle',':','Color','r',...
        'LineWidth',2.5); title('Inflation','FontSize',14 ) ; xlim([-0.5 0.2]);
   hold off 
subplot(1,3,3); [density,x1]=ksdensity(i_sim);
       [density,x2]=ksdensity(i1_sim);
   hold on
        plot(x1,density,'LineStyle','-','Color','b',...
        'LineWidth',2.5);  
       plot(x2,density,'LineStyle',':','Color','r',...
        'LineWidth',2.5); title('Interest Rate','FontSize',14 ) ;
     xlim([-2 2]); xlabel('Interest Rate','FontSize',14);
   hold off 
