...
switch model
 ...
 case 'Taylor_ZLB' 
     ...
     load('./output/Taylor_pf_ZLB.mat', 'Taylor_pf_g');
      ...
      if regime ==1
      PF_y_pf = PF1_y_Taylor_pf;
      PF_i_pf = PF1_i_Taylor_pf;
      PF_pi_pf = PF1_pi_Taylor_pf;
      else
     PF_y_pf = PF0_y_Taylor_pf;
     PF_i_pf = PF0_i_Taylor_pf;
     PF_pi_pf = PF0_pi_Taylor_pf;
      end
...
%   u_sim = -0.00;
 ...
 pi_0 = interp2(u_ZLB, g_ZLB, PF_pi_stc, 0, 0, 'cubic');
 i_0 = interp2(u_ZLB, g_ZLB, PF_i_stc, 0, 0, 'cubic');
 y_0 = interp2(u_ZLB, g_ZLB, PF_y_stc, 0, 0, 'cubic');
 pi_ss = interp2(u_ZLB, g_ZLB, PF_pi_pf, 0, 0, 'cubic');
 i_ss = interp2(u_ZLB, g_ZLB, PF_i_pf, 0, 0, 'cubic');
 y_ss = interp2(u_ZLB, g_ZLB, PF_y_pf, 0, 0, 'cubic');
  
  for j = 1:N
     
     pi2(j) = interp2(u_pf, g_pf, PF_pi_pf, u_sim, g_sim(j), 'cubic');
     y2(j)  = interp2(u_pf, g_pf, PF_y_pf, u_sim, g_sim(j), 'cubic');
